...
const locatorGenerators = require('./locatorGenerators.js');
  ...
  return parts.map((t) => {
    ...
    t = t.replace(/(?:r)\$(\d+)(i)?/g, (_, ordinal, suffix) => {
      ...
      if (t.startsWith("internal:attr") || t.startsWith("internal:testid") || t.startsWith("internal:role"))
        return stringUtils.escapeForAttributeSelector(new RegExp(param.text), false) + (suffix || "");
      return stringUtils.escapeForTextSelector(new RegExp(param.text, suffix), false);
      ...
      if (t.startsWith("internal:testid"))
        return stringUtils.escapeForAttributeSelector(param.text, true);
      if (t.startsWith("internal:attr") || t.startsWith("internal:role"))
        return stringUtils.escapeForAttributeSelector(param.text, suffix === "s");
      return stringUtils.escapeForTextSelector(param.text, suffix === "s");
    });
    return t;
  }).join(" >> ");
}
...
function unsafeLocatorOrSelectorAsSelector(language, locator, testIdAttributeName) {
  try {
    selectorParser.parseSelector(locator);
    return locator;
  } catch (e) {
  }
  const { selector, preferredQuote } = parseLocator(locator, testIdAttributeName);
  const locators = locatorGenerators.asLocators(language, selector, void 0, void 0, preferredQuote);
