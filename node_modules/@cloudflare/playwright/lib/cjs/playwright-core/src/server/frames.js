...
class Frame extends instrumentation.SdkObject {
  ...
  async goto(metadata, url, options = {}) {
    const constructedNavigationURL = urlMatch.constructURLBasedOnBaseURL(this._page._browserContext._options.baseURL, url);
  ...
  async _raceWithCSPError(func) {
    ...
    const errorPromise = new Promise((resolve) => {
      listeners.push(eventsHelper.eventsHelper.addEventListener(this._page._browserContext, browserContext.BrowserContext.Events.Console, (message) => {
  ...
  async retryWithProgressAndTimeouts(progress, timeouts, action) {
    ...
    while (progress.isRunning()) {
      ...
      if (timeout) {
        ...
        await manualPromise.LongStandingScope.raceMultiple([
          this._page.openScope,
          this._detachedScope
        ], actionPromise);
      }
  ...
  async isVisibleInternal(selector, options = {}, scope) {
    try {
      ...
      if (javascript.isJavaScriptErrorInEvaluate(e) || cssParser.isInvalidSelectorError(e) || protocolError.isSessionClosedError(e))
        throw e;
      return false;
    }
  }
  ...
  async _expectImpl(metadata, selector, options) {
    ...
    try {
      ...
      const start = timeout > 0 ? time.monotonicTime() : 0;
      await new progress.ProgressController(metadata, this).run(async (progress) => {
        progress.log(`${metadata.apiName}${timeout ? ` with timeout ${timeout}ms` : ""}`);
        progress.log(`waiting for ${this._asLocator(selector)}`);
        await this._page.performActionPreChecks(progress);
      }, timeout);
      try {
        const resultOneShot = await new progress.ProgressController(metadata, this).run(async (progress) => {
      ...
      if (timeout > 0) {
        const elapsed = time.monotonicTime() - start;
        timeout -= elapsed;
      }
      if (timeout < 0)
        return { matches: options.isNot, log: callLog.compressCallLog(metadata.log), timedOut: true, received: lastIntermediateResult.received };
      return await new progress.ProgressController(metadata, this).run(async (progress) => {
      ...
      if (javascript.isJavaScriptErrorInEvaluate(e) || cssParser.isInvalidSelectorError(e))
        throw e;
      const result = { matches: options.isNot, log: callLog.compressCallLog(metadata.log) };
  ...
  async waitForFunctionValueInUtility(progress, pageFunction) {
    ...
    const handle = await this._waitForFunctionExpression(instrumentation.serverSideCallMetadata(), expression, true, void 0, { timeout: progress.timeUntilDeadline() }, "utility");
  ...
  _asLocator(selector) {
    return locatorGenerators.asLocator(this._page.attribution.playwright.options.sdkLanguage, selector);
  }
}
class SignalBarrier {
  ...
  async addFrameNavigation(frame) {
    if (frame.parentFrame())
      return;
    this.retain();
    const waiter = helper.helper.waitForEvent(null, frame, Frame.Events.InternalNavigation, (e) => {
      if (!e.isPublic)
        return false;
