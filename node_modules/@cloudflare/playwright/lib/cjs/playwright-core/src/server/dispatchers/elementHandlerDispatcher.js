...
class ElementHandleDispatcher extends jsHandleDispatcher.JSHandleDispatcher {
  ...
  static from(scope, handle) {
    return dispatcher.existingDispatcher(handle) || new ElementHandleDispatcher(scope, handle);
  }
  ...
  static fromJSHandle(scope, handle) {
    const result = dispatcher.existingDispatcher(handle);
    if (result)
      return result;
    return handle.asElement() ? new ElementHandleDispatcher(scope, handle.asElement()) : new jsHandleDispatcher.JSHandleDispatcher(scope, handle);
  }
  ...
  async dispatchEvent(params, metadata) {
    await this._elementHandle.dispatchEvent(metadata, params.type, jsHandleDispatcher.parseArgument(params.eventInit));
  }
  ...
  async evalOnSelector(params, metadata) {
    return { value: jsHandleDispatcher.serializeResult(await this._elementHandle.evalOnSelector(params.selector, !!params.strict, params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg))) };
  }
  async evalOnSelectorAll(params, metadata) {
    return { value: jsHandleDispatcher.serializeResult(await this._elementHandle.evalOnSelectorAll(params.selector, params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg))) };
  }
  ...
  _browserContextDispatcher() {
    ...
    if (scope instanceof browserContextDispatcher.BrowserContextDispatcher)
      return scope;
    if (scope instanceof pageDispatcher.PageDispatcher)
      return scope.parentScope();
    if (scope instanceof pageDispatcher.WorkerDispatcher || scope instanceof frameDispatcher.FrameDispatcher) {
