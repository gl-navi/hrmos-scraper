...
const imageChannel = require('./imageChannel.js');
...
function compare(actual, expected, diff, width, height, options = {}) {
  ...
  const [r1, g1, b1] = imageChannel.ImageChannel.intoRGB(width, height, expected, {
    paddingSize,
    paddingColorEven,
    paddingColorOdd
  });
  const [r2, g2, b2] = imageChannel.ImageChannel.intoRGB(width, height, actual, {
    paddingSize,
    paddingColorEven,
    paddingColorOdd
  });
  ...
  const drawGrayPixel = diff ? (x, y) => {
    const gray = colorUtils.rgb2gray(r1.get(x, y), g1.get(x, y), b1.get(x, y));
    const value = colorUtils.blendWithWhite(gray, 0.1);
  ...
  for (let y = paddingSize; y < r1.height - paddingSize; ++y) {
    for (let x = paddingSize; x < r1.width - paddingSize; ++x) {
      ...
      const delta = colorUtils.colorDeltaE94(
      ...
      if (!fastR || !fastG || !fastB) {
        fastR = new stats.FastStats(r1, r2);
        fastG = new stats.FastStats(g1, g2);
        fastB = new stats.FastStats(b1, b2);
      }
      ...
      const ssimRGB = (stats.ssim(fastR, ssimX1, ssimY1, ssimX2, ssimY2) + stats.ssim(fastG, ssimX1, ssimY1, ssimX2, ssimY2) + stats.ssim(fastB, ssimX1, ssimY1, ssimX2, ssimY2)) / 3;
