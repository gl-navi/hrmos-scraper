...
class FFBrowserContext extends browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    const promises = [
      super._initialize(),
      this._browser.session.send("Browser.addBinding", { browserContextId: this._browserContextId, name: page.PageBinding.kPlaywrightBinding, script: "" }),
  ...
  async _updateInitScripts() {
    ...
    await this._browser.session.send("Browser.setInitScripts", { browserContextId: this._browserContextId, scripts: [page.kBuiltinsScript.source, ...bindingScripts, ...initScripts].map((script) => ({ script })) });
  }
