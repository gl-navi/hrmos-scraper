...
const linuxUtils = require('./linuxUtils.js');
...
function determineUserAgent() {
  ...
  if (process.platform === "win32") {
    ...
    const version = childProcess.execSync("sw_vers -productVersion", { stdio: ["ignore", "pipe", "ignore"] }).toString().trim().split(".");
    osIdentifier = "macOS";
    osVersion = `${version[0]}.${version[1]}`;
  } else if (process.platform === "linux") {
    const distroInfo = linuxUtils.getLinuxDistributionInfoSync();
    if (distroInfo) {
...
function getPlaywrightVersion(majorMinorOnly = false) {
  const version = process.env.PW_VERSION_OVERRIDE || _package.default.version;
