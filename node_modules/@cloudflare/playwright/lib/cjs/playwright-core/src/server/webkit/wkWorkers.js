...
class WKWorkers {
  ...
  setSession(session) {
    ...
    this._sessionListeners = [
      eventsHelper.eventsHelper.addEventListener(session, "Worker.workerCreated", (event) => {
        ...
        worker._createExecutionContext(new wkExecutionContext.WKExecutionContext(workerSession, void 0));
  ...
  async _onConsoleMessage(worker, event) {
    ...
    const handles = (parameters || []).map((p) => {
      return wkExecutionContext.createHandle(worker._existingExecutionContext, p);
    });
    const location = {
      url: url || "",
