...
class ElementHandle extends javascript.JSHandle {
  ...
  async setInputFiles(metadata, params) {
    const inputFileItems = await fileUploadUtils.prepareFilesForUpload(this._frame, params);
    const controller = new progress.ProgressController(metadata, this);
    return controller.run(async (progress) => {
      await this._markAsTargetElement(metadata);
  ...
  async _setInputFiles(progress, items) {
    ...
    if (localPaths || localDirectory) {
      ...
      await Promise.all(localPathsOrDirectory.map((localPath) => fs.default.promises.access(localPath, fs.default.constants.F_OK)));
