...
class FrameSession {
  ...
  async _initialize(hasUIWindow) {
    ...
    const promises = [
      ...
      this._client.send("Runtime.addBinding", { name: page.PageBinding.kPlaywrightBinding }),
  ...
  _onAttachedToTarget(event) {
    ...
    session.once("Runtime.executionContextCreated", async (event2) => {
      worker._createExecutionContext(new crExecutionContext.CRExecutionContext(session, event2.context));
    });
    ...
    session.on("Runtime.consoleAPICalled", (event2) => {
      const args = event2.args.map((o) => crExecutionContext.createHandle(worker._existingExecutionContext, o));
      this._page._addConsoleMessage(event2.type, args, crProtocolHelper.toConsoleMessageLocation(event2.stackTrace));
    });
  ...
  async _onConsoleAPI(event) {
    ...
    this._page._addConsoleMessage(event.type, values, crProtocolHelper.toConsoleMessageLocation(event.stackTrace));
  }
  ...
  async _createVideoRecorder(screencastId, options) {
    ...
    const ffmpegPath = index.registry.findExecutable("ffmpeg").executablePathOrDie(this._page.attribution.playwright.options.sdkLanguage);
