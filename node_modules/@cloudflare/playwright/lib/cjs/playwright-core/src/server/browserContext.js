...
const builtins = require('../utils/isomorphic/builtins.js');
...
const storageScript = require('./storageScript.js');
...
class BrowserContext extends instrumentation.SdkObject {
  constructor(browser, options, browserContextId) {
    ...
    if (this._options.recordHar)
      this._harRecorders.set("", new harRecorder.HarRecorder(this, null, this._options.recordHar));
  ...
  async _initialize() {
    ...
    if (debug.debugMode() === "inspector")
      await recorder.Recorder.show(this, recorderApp.RecorderApp.factory(this), { pauseOnNextStatement: true });
    if (this._debugger.isPaused())
      recorder.Recorder.showInspectorNoReply(this, recorderApp.RecorderApp.factory(this));
...
`);
  ...
  async addInitScript(source, name) {
    const initScript = new page.InitScript(source, false, name);
  ...
  async newPage(metadata) {
    const page$1 = await this.doCreateNewPage();
    if (metadata.isServerSide)
      page$1.markAsServerSideOnly();
    const pageOrError = await page$1.waitForInitializedOrError();
  ...
  async storageState(indexedDB = false) {
    ...
    let collectScript = `(${storageScript.collect})(${utilityScriptSerializers.source}, (${builtins.builtins})(), ${this._browser.options.name === "firefox"}, ${indexedDB})`;
    ...
    if (originsToSave.size) {
      const internalMetadata = instrumentation.serverSideCallMetadata();
    ...
    try {
      ...
      if (state.origins && state.origins.length) {
        ...
        for (const originState of state.origins) {
          ...
          const builtinsScript = `((__name => (${builtins.builtins}))(t => t))`;
