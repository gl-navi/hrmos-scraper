...
${e.stack}`);
  ...
  async _installMSEdgeChannel(channel, scripts) {
    const scriptArgs = [];
    if (process.platform !== "linux") {
      const products = lowercaseAllKeys(JSON.parse(await network.fetchData({ url: "https://edgeupdates.microsoft.com/api/products" })));
      const productName = {
        "msedge": "Stable",
        "msedge-beta": "Beta",
        "msedge-dev": "Dev"
    ...
    for (const fileName of await fs.default.promises.readdir(linksDir)) {
      ...
      try {
        ...
        for (const browserName of allDownloadable) {
          ...
          if (!shouldHaveMarkerFile || await fileUtils.existsAsync(browserDirectoryToMarkerFilePath(usedBrowserPath)))
            usedBrowserPaths.add(usedBrowserPath);
        }
      } catch (e) {
        await fs.default.promises.unlink(linkPath).catch((e2) => {
        });
      }
    }
