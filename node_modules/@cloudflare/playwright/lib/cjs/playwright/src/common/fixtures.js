    ...
    for (const entry of Object.entries(fixtures)) {
      ...
      if (previous && options) {
        if (previous.scope !== options.scope) {
          this._addLoadError(`Fixture "${name}" has already been registered as a { scope: '${previous.scope}' } fixture defined in ${util.formatLocation(previous.location)}.`, location);
          continue;
        }
        if (previous.auto !== options.auto) {
          this._addLoadError(`Fixture "${name}" has already been registered as a { auto: '${previous.scope}' } fixture defined in ${util.formatLocation(previous.location)}.`, location);
          continue;
        }
      } else if (previous) {
...
function formatPotentiallyInternalLocation(location) {
  const isUserFixture = location && util.filterStackFile(location.file);
  return isUserFixture ? util.formatLocation(location) : "<builtin>";
}
