...
class SnapshotHelper {
  constructor(testInfo, matcherName, locator, anonymousSnapshotExtension, configOptions, nameOrOptions, optOptions) {
    ...
    if (!name) {
      ...
      expectedPathSegments = [fileUtils.sanitizeForFilePath(util.trimLongString(fullTitleWithoutSpec)) + "." + anonymousSnapshotExtension];
      const sanitizedName = fileUtils.sanitizeForFilePath(util.trimLongString(fullTitleWithoutSpec, util.windowsFilesystemFriendlyLength)) + "." + anonymousSnapshotExtension;
      outputBasePath = testInfo._getOutputPath(sanitizedName);
      this.attachmentBaseName = sanitizedName;
    } else {
      expectedPathSegments = Array.isArray(name) ? name : [util.sanitizeFilePathBeforeExtension(name)];
      const joinedName = Array.isArray(name) ? name.join(path.sep) : util.sanitizeFilePathBeforeExtension(util.trimLongString(name, util.windowsFilesystemFriendlyLength));
      ...
      const sanitizedName = index > 1 ? util.addSuffixToFilePath(joinedName, `-${index - 1}`) : joinedName;
      outputBasePath = testInfo._getOutputPath(sanitizedName);
      this.attachmentBaseName = sanitizedName;
    }
    ...
    this.mimeType = utilsBundle.mime.getType(path.basename(this.expectedPath)) ?? "application/octet-string";
  ...
  handleDifferent(actual, expected, previous, diff, header, diffError, log, step) {
    ...
    if (expected !== void 0) {
      ...
      step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-expected"), contentType: this.mimeType, path: this.expectedPath });
      output.push(`
Expected: ${utilsBundle.colors.yellow(this.expectedPath)}`);
    }
    if (previous !== void 0) {
      ...
      output.push(`Previous: ${utilsBundle.colors.yellow(this.previousPath)}`);
    }
    if (actual !== void 0) {
      writeFileSync(this.actualPath, actual);
      step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-actual"), contentType: this.mimeType, path: this.actualPath });
      output.push(`Received: ${utilsBundle.colors.yellow(this.actualPath)}`);
    }
    if (diff !== void 0) {
      ...
      output.push(`    Diff: ${utilsBundle.colors.yellow(this.diffPath)}`);
    }
    if (log?.length)
