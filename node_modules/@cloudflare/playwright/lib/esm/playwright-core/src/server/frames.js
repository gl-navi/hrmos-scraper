...
class Frame extends SdkObject {
  ...
  async _raceWithCSPError(func) {
    ...
    const errorPromise = new Promise((resolve) => {
      listeners.push(eventsHelper.addEventListener(this._page._browserContext, BrowserContext.Events.Console, (message) => {
  ...
  async _expectImpl(metadata, selector, options) {
    ...
    try {
      ...
      if (isJavaScriptErrorInEvaluate(e) || isInvalidSelectorError(e))
        throw e;
      const result = { matches: options.isNot, log: compressCallLog(metadata.log) };
  ...
  async waitForFunctionValueInUtility(progress, pageFunction) {
    ...
    const handle = await this._waitForFunctionExpression(serverSideCallMetadata(), expression, true, void 0, { timeout: progress.timeUntilDeadline() }, "utility");
